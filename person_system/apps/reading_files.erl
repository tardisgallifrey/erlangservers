...
start() ->
    {ok, _} = application:start(person_prod_app),
    PrivDir = code:priv_dir(person_prod_app),
    io:format("Priv Directory: ~s~n", [PrivDir]),
    DetFile = filename:join([PrivDir, "your_dets_files", "file1.dets"]),
    %% Now you can use DetFile for opening your DETS file
    ok.
...
open_dets_file() ->
    Dir = "/home/user/",
    FileName = filename:join([Dir, "your_file.dets"]),
    
    case ensure_directory_exists(Dir) of
        ok ->
